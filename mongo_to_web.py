...
def filtered_data():
    if request.method == 'POST':
        ...
        filtered_crypto_data = list(collection.find({'symbol': {'$in': selected_crypto_symbols}}))

        # Pagination parameters
        page, per_page, offset = get_page_args(page_parameter='page', per_page_parameter='per_page')
        total = len(filtered_crypto_data)

        pagination_crypto_data = filtered_crypto_data[offset:offset + per_page]

        pagination = Pagination(page=page, per_page=per_page, total=total, css_framework='bootstrap4')

        return render_template('filtered.html', crypto_data=pagination_crypto_data, pagination=pagination, per_page=per_page, total=total, cryptos=cryptos)
    else:
        # Handle GET request
